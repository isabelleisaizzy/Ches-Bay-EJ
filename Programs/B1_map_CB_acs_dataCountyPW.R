...
data_path <- here('data') #relative path to the data folder
  ...
  #load CB county shapefile
  cb_county_sf <- st_read(here(data_path, 'watershed boundary', 'Chesapeake_Bay_Watershed_Boundary_2020county.shp'))
...
# Export this shapefile for use in ArcGIS
#   Note, for this step, comment out the geometry drop above
   # st_write(cb_acs_df_clean, here(data_path,"cb_acs_df_cleanF.shp" ),append=FALSE)


#2. CREATE MAPS
  #Percent Black
  cb_county_sf_pctblack <- cb_county_sf %>%
  ...
  # Why not just join the whole thing? Memory space?
  cb_county_sf_Demos <- cb_county_sf %>%
    left_join(cb_acs_df_clean , by = 'GEOID')
  st_write(cb_county_sf_Demos, here(data_path,"cb_county_sf_Demos.shp" ),append=FALSE)

  mapView( cb_county_sf_Demos,
          zcol = 'minority_black',
          alpha.regions = 0.8,
          # alpha = 0.1,
          layer.name = '% Black')
# Hrm, the BG polygons are small, so the lines between them tend to dominate the picture..
  # need to mess with fixing.
