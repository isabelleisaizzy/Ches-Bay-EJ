...
data_path <- here('data') #relative path to the data folder
  ...
  cb_bg_sf <- map_dfr(state_vec, function(my_state){
    block_groups(state = my_state)
  })
  ...
  cb_sf <- st_read(here(data_path, 'watershed boundary', 'Chesapeake_Bay_Watershed_Boundary.shp')) %>%
    st_transform(st_crs(cb_bg_sf)) #reproject to have same crs as cb_bg_sf
...
#STEP 2) CREATE LIST OF BLOCKGROUPS INSIDE THE WATERSHED
  cb_bg_vec <- unique(cb_bg_sf$GEOID)

#STEP 3) SUBSET ACS DATASET TO LIST OF BLOCKGROUPS
  cb_acs_df <- data %>%
    filter(GEOID %in% cb_bg_vec)

#STEP 4) SAVE THE DATASETS
  st_write(cb_bg_sf,
           here(data_path, 'watershed boundary', 'Chesapeake_Bay_Watershed_Boundary_2020blockgroups.shp'))

  save(cb_acs_df,
       file = file.path(data_path, 'acs_data/CB_acs_data_2020_block group.RData'))
